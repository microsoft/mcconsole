...
class SessionManager:
    def __init__(self):
        ...
        # Start a background thread to compare local vs remote version
    ...
    def check_for_update(self):
        try:
            remote_url = "https://McqueenWebBackup.redmond.corp.microsoft.com:5002/check_mcconsole"
            context = ssl._create_unverified_context()  # disable SSL certificate verification
            with urllib.request.urlopen(remote_url, context=context, timeout=10) as response:
                remote_ver = response.read().decode().strip()
            
            print("Local version raw:", self.local_version)
            print("Remote version raw:", remote_ver)

            local_ver_tuple = self.parse_version(self.local_version)
            remote_ver_tuple = self.parse_version(remote_ver)

            print("Local version tuple:", local_ver_tuple)
            print("Remote version tuple:", remote_ver_tuple)

            if remote_ver_tuple > local_ver_tuple:
                print(f"Upgrade available: remote version {remote_ver} is higher than local version {self.local_version}")
                self.root.after(0, self.add_upgrade_button, remote_ver)
            else:
    ...
    def add_upgrade_button(self, remote_ver):
        """
        Add an 'Upgrade to vXXX' button in the top-right area of the UI
        that will let the user download the zip and update.
        """
        ...
        #     self.settings_frame,
        #     text=f"Upgrade to v{remote_ver}",
        #     bg="#FF9800", fg="#FFFFFF",
        #     font=("Consolas", 10),
        #     command=self.perform_upgrade
        # )
        ...
        # Pack to the RIGHT so it appears to the left of the link
        self.upgrade_button.pack(side=RIGHT, padx=(0, 5))
        print("Upgrade button added to UI.")
        self.create_tooltip(self.upgrade_button, f"Download and install latest version - {remote_ver}")

    def perform_upgrade(self):
        """
        Download the update ZIP in a background thread (UI stays responsive,
        indeterminate/determinate progress), then generate & launch a batch
        that uses 7-Zip to extract (errors logged but ignored), always
        relaunches McConsole.bat, and cleans up.
        """
        ...
        # 2) URLs & paths
        upgrade_url   = "https://McqueenWebBackup.redmond.corp.microsoft.com:5002/download_mcconsole"
        base_dir      = Path(__file__).parent.parent
        zip_path      = base_dir / "McConsole_update.zip"
        exe_name      = "McConsole_secure.exe"
        exe_path      = base_dir / exe_name
        sevenzip_path = base_dir / "bins" / "7-Zip" / "7z.exe"
        logs_dir      = base_dir / "logs" / "mcconsole"
        logs_dir.mkdir(parents=True, exist_ok=True)
        log_file      = logs_dir / "upgrade.log"

        # 3) build the download dialog
        dlg = tk.Toplevel(self.root)
        dlg.title("Downloading update…")
        dlg.geometry("400x120")
        dlg.transient(self.root); dlg.grab_set()

        ttk.Label(dlg, text="Downloading update…").pack(pady=(10,0))
        bar = ttk.Progressbar(dlg, length=360, mode="determinate")
        bar.pack(pady=10)
        pct = ttk.Label(dlg, text="0%")
        pct.pack()
        ...
        def finalize_and_launch():
            # 1) Close the download dialog if it's still open
            try:
                dlg.destroy()
            except Exception:
                pass

            # 2) Determine which EXE to launch
            exe_candidates = ["McConsole_secure.exe", "McConsole.exe"]
            for candidate in exe_candidates:
                if (base_dir / candidate).exists():
                    exe_name = candidate
                    break
            else:
                exe_name = exe_candidates[-1]   # fallback even if it doesn't exist

            # 3) Build the batch script
            batch_path = base_dir / "upgrade.bat"
            batch_content = f"""@echo off
rem *** Configuration: adjust these paths to match your installation ***
set "BASE_DIR=C:\\McQueen"
set "ZIP_PATH=%BASE_DIR%\\McConsole_update.zip"
set "LOG_FILE=%BASE_DIR%\\logs\\mcconsole\\upgrade.log"
set "SEVENZIP_PATH=%BASE_DIR%\\bins\\7-Zip\\7z.exe"

echo Terminating any python processes running mcconsole.py...
powershell -NoProfile -Command "Get-CimInstance Win32_Process ^| Where-Object {{ $_.CommandLine -like '*mcconsole.py*' }} ^| ForEach-Object {{ Stop-Process -Id $_.ProcessId -Force }}" >nul 2>&1

echo Stopping {exe_name}...
taskkill /F /IM {exe_name} /T >nul 2>&1
timeout /t 5 /nobreak >nul

echo Extracting update (logging to %LOG_FILE%)...
"%SEVENZIP_PATH%" x "%ZIP_PATH%" -o"%BASE_DIR%" -y -bb2 -bsp1 > "%LOG_FILE%" 2>&1
if %ERRORLEVEL% NEQ 0 echo Warning: extraction errors, but proceeding.

echo Relaunching via shortcut or batch...
if exist "%~dp0McConsole.lnk" (
    echo Launching McConsole.lnk shortcut...
    start "" "%~dp0McConsole.lnk"
) else if exist "%~dp0console\\mcconsole.bat" (
    echo Launching console\\mcconsole.bat...
    start "" "%~dp0console\\mcconsole.bat"
) else (
    echo Launching {exe_name}...
    start "" "%~dp0{exe_name}"
)

echo Cleaning up update ZIP...
del "%ZIP_PATH%"
...
"""
            ...
            subprocess.Popen(
                f'start "McConsole Upgrade" cmd /k "{batch_path}"',
                cwd=str(base_dir), shell=True
            )

            # 5) Tear down the GUI cleanly
        ...
        def download_thread():
            try:
                ...
                with urllib.request.urlopen(upgrade_url, context=ctx, timeout=10) as resp:
                    raw_len = resp.getheader("Content-Length")
                    if raw_len and raw_len.isdigit():
                        total = int(raw_len)
                        # determinate: percent 0→100
                        self.root.after(0, lambda: bar.config(mode="determinate", maximum=100))
                        self.root.after(0, lambda: pct.config(text="0%"))
                    else:
                        total = None
                        # indeterminate spinner
                        self.root.after(0, lambda: bar.config(mode="indeterminate"))
                        self.root.after(0, bar.start)
                    ...
                    with open(zip_path, "wb") as out:
                        while True:
                            ...
                            if total:
                                ...
                                self.root.after(0, lambda p=p: (bar.config(value=p), pct.config(text=f"{p}%")))
                            else:
                                mb = downloaded / (1024*1024)
                                self.root.after(0, lambda mb=mb: pct.config(text=f"{mb:.1f} MB"))

                # done → finalize
                self.root.after(0, finalize_and_launch)
